...
public class ReadCD2048Command extends AbstractCommand {
  ...
  public void executeTask() throws IOException, PS3NetSrvException {
    if (sectorCount > MAX_SECTORS) {
      throw new IllegalArgumentException(PS3NetSrvApp.getAppContext().getString(R.string.error_too_many_sectors));
      // TODO: VERIFICAR O QUE PODE SER DEVOLVIDO COMO RESPOSTA
    }
    java.util.Set<IFile> files = ctx.getFile();
    if (files == null || files.isEmpty()) {
      throw new IllegalArgumentException(PS3NetSrvApp.getAppContext().getString(R.string.error_file_null));
    }
